...
同时新增[笔记本联网断网与离线实车操作手册](../10_实车接入与动态场景重规划_2026-08-31/笔记本联网断网与离线实车操作手册_2026-09-01.md)，明确公网准备、单网卡切换到底盘 Wi-Fi、双网络路由、CPU Docker、D435i USB、离线 rosbag 采集、恢复公网和 Git 归档顺序。运动期间只保留底盘局域网；没有真实编码器 `/odom` 和 `odom→base_link` 时仍不得发送 Nav2 目标。
...
4. 按笔记本联网/断网手册完成移动电脑迁移和现场网络切换演练；
5. 动态安全边界明确后，再进入真实 D435i 的低速静态导航和动态安全停车验证。
