...
> 历史基线说明：本目录记录的三次回归使用的是
> `adaptive_goal_line_045`，仅作为旧动态管线诊断结果。自 2026-08-31 起，项目当前
> 工作基础切换为四点闭环
> `adaptive_goal_line_050_recovery_v13_line_tiebreaker`；旧结果不改写、不覆盖，也不
> 能直接作为新的动态场景03最终验收。新的动态场景设计和静态实车接入准备见
...
| 本历史回归 profile | `adaptive_goal_line_045`，未盲目调参 |
| 当前后续工作基础 | `adaptive_goal_line_050_recovery_v13_line_tiebreaker` |
