...
> **2026-08-31 当前状态补充：** 四点闭环 v13
> `adaptive_goal_line_050_recovery_v13_line_tiebreaker` 已冻结为当前新实验和真实
> 实车启动的工作基础；`adaptive_goal_line_045` 保留为通用对照及旧场景 01/场景 03
> 历史复现。场景 02 正式结果和旧场景 03 动态结果均不改写。阶段4已经完成真实
> D435i/WATER II-S 的软件接入准备，但尚未产生真实底盘成功率、耗时、轨迹或碰撞结果。
...
| --- | --- | --- | --- |
| adaptive_goal_line_045 | 通用对照/历史基线 | 否 | 旧场景复现和明确标注的比较 |
| fast_goalline_045_v4 | large 旧 benchmark | 是 | 历史最快结果和方法演进对照 |
| adaptive_goal_line_050_recovery_v13_line_tiebreaker | 四点闭环与后续工作基础 | 否 | 当前新实验和实车启动 profile；场景 02 正式验收 profile |
...
1. 创建新的动态场景 03，只改变一个环境因素；
2. 新实验使用四点闭环 v13 做 smoke test；045 只用于明确标注的历史/对照复现；
3. 再按相同记录规则跑三次；
