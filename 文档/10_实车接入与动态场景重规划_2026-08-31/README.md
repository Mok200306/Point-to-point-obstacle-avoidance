...
更新时间：2026-09-01
当前状态：WATER SDK 已完成 ROS 2 bridge 软件接入和 mock 验证，尚未进行真实底盘运动和真实 D435i 验收。
...
- [笔记本联网断网与离线实车操作手册](笔记本联网断网与离线实车操作手册_2026-09-01.md)
...
- [WATER SDK ROS 2 桥接说明](WATER_SDK_ROS2桥接接入说明_2026-09-01.md)
...
8. 新增 WATER II-S 参数记录，包括尺寸、网络地址、TCP 端口、控制接口范围、里程计
   边界和初始限速。
9. 将厂家 WATER V5.1 SDK 保留为独立依赖，并新增 ROS 2 bridge：只允许
   `/cmd_vel_safe → set_velocity()`；默认诊断模式不转发速度、不发布暂定 `/odom`，并已用
   本机 mock 底盘验证状态链、速度链和退出清理。

10. 新增笔记本联网、断网与离线实车现场手册，明确 CPU Docker、D435i USB、底盘局域网、
    单网卡/双网卡路由、公网准备和恢复、rosbag 记录、H0-H8 放行以及真实实车结果边界。

## 笔记本现场网络结论

移动笔记本第一次承担完整 ROS 2 控制链：D435i 用 USB 3.0 接笔记本，笔记本用底盘 Wi-Fi
或网线连接 WATER II-S。公网只用于下载、构建、备份和 GitHub；实时导航期间必须保持底盘
局域网，单无线网卡时应断开公网 Wi-Fi。实验结束并确认机器人安全停止、ROS 和底盘驱动
关闭后，才能切回公网。

详细的逐终端命令、预期结果、故障处理和记录模板见
[笔记本联网断网与离线实车操作手册](笔记本联网断网与离线实车操作手册_2026-09-01.md)。

该手册不改变当前安全前提：真实编码器 `/odom`、`odom→base_link`、watchdog、硬件急停
和唯一 `/cmd_vel_safe` 控制拥有者未确认前，不得发送 Nav2 目标。
...
- WATER SDK ROS 2 bridge 已完成软件接入，但这不等于真实底盘已接入；真实成功率、耗时、
  轨迹和 contacts 仍全部为“未测”；
...
本阶段完成的是“SDK 软件桥接可以开始按 H0-H8 验收”，不是“已经完成 H0-H8”，也不是
“真实底盘已经完成静态导航”。
