...
# Isolated Gate 1 runner. It keeps the existing RPP/navigation implementation
...
if [[ "$nav2_params" == /* ]]; then
  case "$nav2_params" in
    "$repo_root"/*)
      nav2_params_host="$nav2_params"
      nav2_params="${nav2_params#"$repo_root/"}"
      ;;
    *) printf 'Nav2 params must be inside repository: %s\n' "$nav2_params" >&2; exit 2 ;;
  esac
else
...
bag_pid=''
control_pid=''
tmp_label="oracle_gate1_$(basename "$label")_$$"
...
if [[ -f "$artifact_dir/cmd_vel.csv" ]]; then
  compose_exec "source /opt/ros/humble/setup.bash; python3 /workspaces/rtabmap_tb3_nav/experiments/oracle_mppi/scripts/analyze_control_frequency.py --input /workspaces/rtabmap_tb3_nav/${label}/cmd_vel.csv --output /workspaces/rtabmap_tb3_nav/${label}/control_frequency.yaml --expected-period ${expected_control_period}" >"$artifact_dir/control_analysis.log" 2>&1 || true
fi
...
if [[ -f "$artifact_dir/control_frequency.yaml" && -f "$artifact_dir/metrics.yaml" ]]; then
  {
    printf 'control_samples: %s\n' "$(awk -F': ' '$1 == "samples" {print $2}' "$artifact_dir/control_frequency.yaml" | tail -1)"
    printf 'control_interval_samples: %s\n' "$(awk -F': ' '$1 == "interval_samples" {print $2}' "$artifact_dir/control_frequency.yaml" | tail -1)"
    printf 'control_mean_period_s: %s\n' "$(awk -F': ' '$1 == "mean_period_s" {print $2}' "$artifact_dir/control_frequency.yaml" | tail -1)"
    printf 'control_p95_period_s: %s\n' "$(awk -F': ' '$1 == "p95_period_s" {print $2}' "$artifact_dir/control_frequency.yaml" | tail -1)"
    printf 'control_max_period_s: %s\n' "$(awk -F': ' '$1 == "max_period_s" {print $2}' "$artifact_dir/control_frequency.yaml" | tail -1)"
    printf 'control_mean_frequency_hz: %s\n' "$(awk -F': ' '$1 == "mean_frequency_hz" {print $2}' "$artifact_dir/control_frequency.yaml" | tail -1)"
    printf 'control_p95_frequency_hz: %s\n' "$(awk -F': ' '$1 == "p95_frequency_hz" {print $2}' "$artifact_dir/control_frequency.yaml" | tail -1)"
    printf 'control_p95_period_ratio_to_expected: %s\n' "$(awk -F': ' '$1 == "p95_period_ratio_to_expected" {print $2}' "$artifact_dir/control_frequency.yaml" | tail -1)"
  } >>"$artifact_dir/metrics.yaml"
fi
