...
# Gate 1 matrix: A->B and B->A, three isolated runs per direction.
# Complete run directories are reused after validation. Incomplete directories
# are moved to gate1/runner_audit before a new run is started.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      printf '%s\n' \
        'Usage: experiments/oracle_mppi/scripts/run_gate1_matrix.sh' \
        '[--profile NAME] [--nav2-params PATH] [--expected-control-period SEC]' \
        '[--settle-seconds SEC] [--startup-timeout SEC] [--contact-timeout SEC] [--rerun-all]'
      exit 0
      ;;
...
runner='experiments/oracle_mppi/scripts/run_gate1_leg.sh'
summary='experiments/oracle_mppi/gate1/mppi_static.csv'
audit_root='experiments/oracle_mppi/gate1/runner_audit'
nav2_params_snapshot="$(basename "${nav2_params#./}")"
mkdir -p "$(dirname "$summary")" "$audit_root"
printf '%s\n' 'case,run,start_x_m,start_y_m,goal_x_m,goal_y_m,goal_yaw_rad,wrapper_exit,nav2_status,succeeded,wall_duration_s,simulation_duration_s,trajectory_length_m,gazebo_trajectory_length_m,final_xy_error_m,gazebo_non_ground_contact,gazebo_contact_pairs,profile,nav2_params,control_samples,control_mean_period_s,control_p95_period_s,control_max_period_s,control_p95_frequency_hz,control_p95_period_ratio_to_expected,rosbag_present' >"$summary"
...
is_complete() {
  ...
  [[ -f "$metrics" &&
     ...
     -f "$repo_root/$label/$nav2_params_snapshot" &&
     -f "$repo_root/$label/cmd_vel.csv" &&
     -f "$repo_root/$label/control_frequency.yaml" &&
...
append_run() {
  ...
  local label="experiments/oracle_mppi/gate1/$case_name/run_$run_number"
  local metrics="$repo_root/$label/metrics.yaml"
  local experiment="$repo_root/$label/experiment.yaml"
  ...
  local control_samples='null' control_mean='null' control_p95='null' control_max='null' control_p95_hz='null' control_ratio='null'
  ...
  if [[ "$rerun_all" != true ]] && is_complete "$label"; then
    ...
    "$runner" \
      ...
      --profile "$profile" --nav2-params "$nav2_params" --settle-seconds "$settle_seconds" \
  ...
  if [[ -f "$metrics" ]]; then
    ...
    control_samples="$(awk -F': ' '$1 == "control_samples" {print $2}' "$metrics" | tail -1)"
    control_mean="$(awk -F': ' '$1 == "control_mean_period_s" {print $2}' "$metrics" | tail -1)"
    control_p95="$(awk -F': ' '$1 == "control_p95_period_s" {print $2}' "$metrics" | tail -1)"
    control_max="$(awk -F': ' '$1 == "control_max_period_s" {print $2}' "$metrics" | tail -1)"
    control_p95_hz="$(awk -F': ' '$1 == "control_p95_frequency_hz" {print $2}' "$metrics" | tail -1)"
    control_ratio="$(awk -F': ' '$1 == "control_p95_period_ratio_to_expected" {print $2}' "$metrics" | tail -1)"
  fi
  [[ -d "$repo_root/$label/rosbag" ]] && bag='true'

  printf '%s,%s,%s,0.0,%s,0.0,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,"%s",%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "$case_name" "$run_number" "$start_x" "$goal_x" "$goal_yaw" \
    "$runner_exit" "$nav2_status" "$succeeded" "$wall" "$sim" "$path" \
    "$gazebo_path" "$error" "$contact" "$pairs" "$profile" "$nav2_params" \
    "$control_samples" "$control_mean" "$control_p95" "$control_max" \
    "$control_p95_hz" "$control_ratio" "$bag" >>"$summary"
...
successes="$(awk -F, 'NR > 1 && $8 == 0 && $9 == 4 && $10 == "true" && $16 == "false" && $26 == "true" {count++} END {print count + 0}' "$summary")"
total="$(awk 'END {print NR - 1}' "$summary")"
failures=$((total - successes))
printf '\nGate 1 matrix complete: successes=%s total=%s failures=%s\n' "$successes" "$total" "$failures"
printf 'summary=%s\n' "$summary"
